...
def audit_label(label_path: Path, plan_by_doc: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    layout_name = str(label.get("attributes", {}).get("layout_name", ""))
    # Vertical layouts stack a caption within a tall column, so its center can be
    # far from the image/table center even when correctly attached. The caption is
    # linked structurally (nearest preceding image/table) at generation time, so we
    # skip the spatial distance heuristic for vertical layouts and keep only the
    # structural caption checks (missing target / target not found).
    vertical = "_v3_v" in layout_name or "vertical" in layout_name
