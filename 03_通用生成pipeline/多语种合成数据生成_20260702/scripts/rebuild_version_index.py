...
def rebuild(version_dir: Path) -> dict[str, Any]:
    ...
    for cat_dir in category_dirs(version_dir):
        ...
        category_summary[category] = {
            ...
            "pua_blocks": qa_summary.get("total_pua_blocks", 0),
            "unsupported_char_blocks": qa_summary.get("total_unsupported_char_blocks", 0),
        }
    ...
    summary = {
        ...
        "total_pua_blocks": sum(item.get("pua_blocks", 0) for item in all_items),
        "total_unsupported_char_blocks": sum(item.get("unsupported_char_blocks", 0) for item in all_items),
        "unsupported_char_counts": dict(
            sum((collections.Counter(item.get("unsupported_char_counts", {})) for item in all_items), collections.Counter()).most_common(60)
        ),
    ...
    rows = "\n".join(
        ...
        f"{stats['total_blocks']} | {stats['overflow_blocks']} | {stats['out_of_bounds_blocks']} | {stats['pua_blocks']} | {stats['unsupported_char_blocks']} |"
...
- 私用区字符块：{summary['total_pua_blocks']}
- 异常/方框风险字符块：{summary['total_unsupported_char_blocks']}
