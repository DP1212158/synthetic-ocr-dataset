    ...
    return [
        {
            ...
            "name": item["name"],
            "version": version,
            "root": f"02_语种工程资源/{item['language_dir']}/03_合成数据生成_20260702/{version}",
        }
        for item in specs
    ]


def build_manifest(version: str, language_args: list[str] | None = None) -> dict:
    languages = []
    ...
    return {
        "title": f"多语种合成图片浏览 Demo - {version}",
...
def main() -> None:
    ...
    manifest = build_manifest(args.version, args.languages)
    MANIFEST_PATH.parent.mkdir(parents=True, exist_ok=True)
    MANIFEST_PATH.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    language_count = len(manifest["languages"])
    image_count = sum(language["image_count"] for language in manifest["languages"])
    print(f"Wrote {MANIFEST_PATH}")
    print(f"Languages: {language_count}")
    print(f"Images: {image_count}")
    for language in manifest["languages"]:
        print(
            f"- {language['name']} {language['version']}: "
