...
def load_language_profile(path: Path | None) -> dict[str, Any]:
    ...
    profile["font_family"] = "\"Local Arial\", \"Local Arial Unicode\", Arial, Helvetica, sans-serif"
    return profile
...
def form_renderer(b: Builder, tpl: dict[str, Any], records: list[dict[str, str]], rng: random.Random, doc: dict[str, Any], theme: dict[str, str]) -> str:
    ...
    if v == 1:
        ...
        body = f"""{b.block('document_title','Saw Simq','doc-title')}<main class="materials">{rows}</main>{b.block('metadata','handler / date / office','meta footer-meta')}"""
