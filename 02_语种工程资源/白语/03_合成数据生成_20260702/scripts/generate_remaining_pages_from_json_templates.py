#!/usr/bin/env python3
"""Generate Zhuang remaining layout categories from structured JSON templates."""
...
def tibetan_font_faces() -> str:
    ...
    bradley = Path("/System/Library/Fonts/Supplemental/Bradley Hand Bold.ttf").resolve().as_uri()
    return f"""
    ...
    @font-face {{ font-family: 'Local Bradley Hand'; src: url('{bradley}') format('truetype'); font-weight: 700; font-style: normal; font-display: block; }}
    """
...
def historical_renderer(b: Builder, tpl: dict[str, Any], records: list[dict[str, str]], rng: random.Random, doc: dict[str, Any], theme: dict[str, str]) -> str:
    ...
    if v == 1:
        ...
        body = f"""<section class="classic-frame"><header>{b.block('metadata','Gijbon Lijsi','meta')}{b.block('metadata','folio 07','meta')}</header><main class="classic-cols">{para_stack(b, records, rng, 54, 56, 104)}</main><footer class="colophon">{colophon}</footer></section>"""
        ...
        body = f"""<section class="sutra">{b.block('document_title','Guj Sawbon','doc-title')}<main>{para_stack(b, records, rng, 42, 54, 100)}</main><aside>{notes}</aside></section>"""
        ...
        body = f"""<main class="clippings">{b.block('document_title','Lijsi Saw','doc-title')}<section class="main-clip">{b.block('section_title',record_title(records,rng,8,22),'section-title')}{para_stack(b, records, rng, 12, 58, 108)}</section>{clips}</main>"""
        ...
        body = f"""<section class="collation"><header>{b.block('document_title','Sawmaj Gauj','doc-title')}{b.block('metadata','volume 3','meta')}</header><main>{para_stack(b, records, rng, 24, 54, 100)}</main><section class="notes">{notes}</section></section>"""
        ...
        body = f"""<main class="rubbing">{b.block('image','','image-box')}<section>{b.block('document_title','Guj Saw Dak','doc-title')}{b.block('section_title','Gaejgangj','section-title')}{para_stack(b, records, rng, 16, 48, 92)}<div class="catalog">{catalog}</div>{b.block('metadata','catalog / archive / 2026','meta')}</section></main>"""
    ...
    return wrap(doc, body, base_css(page, old, doc["font_family"], 20, doc) + extra, "historical-page")
...
def notice_renderer(b: Builder, tpl: dict[str, Any], records: list[dict[str, str]], rng: random.Random, doc: dict[str, Any], theme: dict[str, str]) -> str:
    ...
    page = tpl["page"]
    if v == 1:
        body = f"""<header class="notice-head">{b.block('metadata','No 2026-07','meta')}{b.block('document_title',doc['title'],'doc-title')}</header><main class="notice-body">{para_stack(b, records, rng, 18, 72, 132)}</main><section class="notice-appendix">{b.block('section_title','Gij Cawxgya','section-title')}{para_stack(b, records, rng, 6, 44, 82, 'small')}</section><footer>{b.block('metadata','Gizci','meta')}{b.block('metadata','2026-07-01','meta')}</footer>"""
        ...
        body = f"""<section class="bulletin">{b.block('document_title','Gvangj Gau','doc-title')}<main>{items}</main><aside>{kv(b,'time','2026-07')}{kv(b,'place',record_title(records,rng,8,18))}{kv(b,'contact',record_title(records,rng,8,18))}</aside></section>"""
        ...
        body = f"""<header class="meeting-head">{b.block('metadata','MEETING','meta')}{b.block('document_title',record_title(records,rng,10,24),'doc-title')}</header><main class="meeting"><section>{kv(b,'Seizgan','2026-07')}{kv(b,'Dihdeiz',record_title(records,rng,8,18))}{kv(b,'Duzgaiq',record_text(records,rng,30,70))}{para_stack(b, records, rng, 5, 42, 78, 'small')}</section><aside>{agenda}</aside></main>{b.block('note',record_text(records,rng,110,180),'para')}"""
        ...
        body = f"""{b.block('document_title','Fujgan Gvangj','doc-title')}<main class="attach-notice">{para_stack(b, records, rng, 12, 66, 124)}<section class="attachments">{b.block('section_title','Fujgan','section-title')}{attachments}</section><section class="notice-signs">{sign_boxes}</section></main>"""
        ...
        body = f"""<section class="public-notice">{b.block('document_title','Public Notice','doc-title')}<main>{para_stack(b, records, rng, 16, 66, 124)}</main><aside>{info}</aside></section>"""
        ...
        body = f"""{b.block('document_title','Seizgan Gvangj','doc-title')}<main class="timeline">{nodes}</main><section class="remarks">{b.block('section_title','Sawmaj','section-title')}{para_stack(b, records, rng, 8, 38, 74, 'small')}</section>"""
    ...
    return wrap(doc, body, base_css(page, theme, doc["font_family"], 20, doc) + extra, "notice-page")
...
def form_renderer(b: Builder, tpl: dict[str, Any], records: list[dict[str, str]], rng: random.Random, doc: dict[str, Any], theme: dict[str, str]) -> str:
    v = int(tpl["variant"])
    page = tpl["page"]
    labels = ["Mingz", "Hauh", "Dihci", "Lienzyau", "Seizgan", "Dihdeiz", "Leiz", "Cingzkuang"]
    if v == 1:
        fields = "".join(kv(b, rng.choice(labels), record_text(records,rng,18,44), "field") for _ in range(28))
        body = f"""{b.block('document_title',doc['title'],'doc-title')}<main class="form-grid">{fields}</main><section class="long-area">{b.block('section_title','Gaejgangj','section-title')}{para_stack(b, records, rng, 4, 50, 95, 'small')}</section><footer>{kv(b,'signature','________')}{kv(b,'date','2026-07')}</footer>"""
        ...
        body = f"""{b.block('document_title','Cingzdan Simq','doc-title')}<section class="field-grid">{fields}</section><section class="request">{para_stack(b, records, rng, 4, 58, 110)}</section><section class="approvals">{approvals}</section>"""
        ...
        body = f"""{b.block('document_title','Saw Simq','doc-title')}<main class="materials">{rows}</main>{b.block('footer','handler / date / office','meta footer-meta')}"""
        ...
        body = f"""{b.block('document_title','Gijman Dan','doc-title')}{b.block('document_subtitle',record_text(records,rng,70,125),'para intro')}<main class="survey">{qs}</main>"""
        ...
        body = f"""<main class="archive-card"><aside>{b.block('metadata','ID 2026-07','meta')}{b.block('image','','photo')}</aside><section>{b.block('document_title','Archive Card','doc-title')}<div class="card-grid">{fields}</div>{b.block('paragraph',record_text(records,rng,80,140),'para')}</section><footer>{kv(b,'review','OK')}{kv(b,'date','2026')}</footer></main>"""
        ...
        body = f"""{b.block('document_title','Danghgi Dan','doc-title')}<main class="signup"><section>{left}</section><section>{right}</section></main><section class="declaration">{b.block('section_title','Cingmingz','section-title')}{para_stack(b, records, rng, 4, 48, 90, 'small')}</section><footer>{kv(b,'sign','________')}{kv(b,'date','2026')}</footer>"""
    ...
    return wrap(doc, body, base_css(page, theme, doc["font_family"], 20, doc) + extra, "form-page")
...
def handwritten_renderer(b: Builder, tpl: dict[str, Any], records: list[dict[str, str]], rng: random.Random, doc: dict[str, Any], theme: dict[str, str]) -> str:
    v = int(tpl["variant"])
    page = tpl["page"]
    hand_family = '"Local Bradley Hand", "Bradley Hand", "Marker Felt", "Comic Sans MS", "Local Arial", cursive'
    if v == 1:
        line_count = 20 if shared_is_vertical_profile(doc) else 34
        lines = "".join(b.block("paragraph", record_text(records,rng,18,44), "hand-line") for _ in range(line_count))
        ...
        body = f"""{b.block('document_title','Sawdiq','doc-title')}<main class="class-notes">{notes}</main><section class="bottom-notes">{bottom_notes}</section>"""
    ...
    return wrap(doc, body, base_css(page, theme, doc["font_family"], 20, doc) + extra, "handwritten-page")
            ...
            html_manifest.append(
                {
                    ...
                    "generator": "remaining_pages_json_renderer_za_v1",
