#!/usr/bin/env python3
"""Generate Tibetan newspaper pages from structured JSON layout trees."""
...
DEFAULT_TITLE = "གསར་འགྱུར་ཤོག་ངོས"
KICKERS = ["ཉིན་རེའི་གསར་འགྱུར", "རིག་གནས · སློབ་གསོ · སྤྱི་ཚོགས", "དམངས་འཚོའི་གསར་འགྱུར", "ཆེད་བསྒྲིགས་ཤོག་ངོས"]
SECTION_TITLES = ["མདོར་བསྡུས", "བརྡ་ཁྱབ", "མྱུར་འཕྲིན", "རིག་གནས", "སློབ་གསོ", "དམིགས་བསལ"]
CONTACT_LABELS = ["འབྲེལ་བ", "ས་གནས", "དུས་ཚོད", "ཁ་བྱང"]
...
def load_language_profile(path: Path | None) -> dict[str, Any]:
    ...
    profile.setdefault("script_note", "藏文横排文本，报纸页 JSON 结构树试点")
...
def select_span(text: str, rng: random.Random, min_len: int, max_len: int) -> str:
    ...
    if len(text) <= max_len:
        return text
    # The source has already had shad punctuation removed, so prefer word-like Tibetan tsek cuts.
...
def random_meta_items(rng: random.Random, variant: int) -> list[str]:
    issue_no = 180 + variant
    return [
        f"ཨང་། {to_tibetan_digits(f'{issue_no:03d}')}",
        to_tibetan_digits(f"2026-07-{variant:02d}"),
        rng.choice(["དེབ་ཐེར", "ཤོག་ངོས་༠༣", "དམིགས་བསལ"]),
    ]
...
def render_masthead(b: Builder, template: dict[str, Any], records: list[dict[str, str]], rng: random.Random, doc: dict[str, Any]) -> str:
    ...
    if region_type == "special_masthead":
        return f"""<header class="special-masthead">
          {b.block('metadata', 'དམིགས་བསལ', 'kicker')}
          {b.block('document_title', title, 'newspaper-name')}
...
def render_v03(b: Builder, template: dict[str, Any], records: list[dict[str, str]], rng: random.Random, doc: dict[str, Any]) -> str:
    ...
    for i in range(34):
        ...
        if i == 5:
            stories.append(f'<section class="micro-ad">{b.block("section_title", "བརྡ་ཁྱབ", "headline")}{b.block("paragraph", record_text(records, rng, 40, 86), "small-para")}</section>')
...
def render_v04(b: Builder, template: dict[str, Any], records: list[dict[str, str]], rng: random.Random, doc: dict[str, Any]) -> str:
    masthead = render_masthead(b, template, records, rng, doc)
    notice = box_section(b, records, rng, "community-box", "བརྡ་ཁྱབ", 7)
    news = '<section>' + "".join(story_block(b, records, rng, paragraphs=1, text_min=54, text_max=112) for _ in range(5)) + "</section>"
    calendar = box_section(b, records, rng, "calendar-box", to_tibetan_digits("06/2026"), 6)
    contact = box_section(b, records, rng, "contact-box", rng.choice(CONTACT_LABELS), 3)
    ad = box_section(b, records, rng, "small-ad-box", "ཆུང་བརྡ", 3)
...
def render_v05(b: Builder, template: dict[str, Any], records: list[dict[str, str]], rng: random.Random, doc: dict[str, Any]) -> str:
    ...
    timeline = '<aside class="sidebar">' + b.block("section_title", "དུས་རིམ", "side-title")
    for i in range(1, 13):
    ...
    timeline += box_section(b, records, rng, "fact-box", "གནད་དོན", 8) + "</aside>"
...
def render_v06(b: Builder, template: dict[str, Any], records: list[dict[str, str]], rng: random.Random, doc: dict[str, Any]) -> str:
    ...
    side = brief_list(b, records, rng, 10, "མྱུར་འཕྲིན")
...
def safe_name(text: str) -> str:
    return re.sub(r"[\\/:*?\"<>|\\s]+", "_", text).strip("_") or "category"
...
def main() -> int:
    ...
    if not args.output_dir and not args.output_root:
        raise SystemExit("provide --output-dir or --output-root")
    ...
    version_name = args.version_name or (Path(args.output_root).name if args.output_root else Path(args.output_dir).name)
    out_dir = (
        Path(args.output_root) / f"{args.start_index:02d}_{safe_name('报纸页')}"
        if args.output_root
        ...
        manifest.append(
            {
                ...
                "generator": "newspaper_json_renderer_bo_v13",
                "doc_no": f"BO-{version_name.upper()}-{args.start_index:02d}-{variant:02d}",
                "date": now_local()[:10],
                "page_width": int(page["width"]),
                "page_height": int(page["height"]),
                "preserve_full_page": variant in {1, 3, 5},
                "rows": None,
                "columns": None,
    ...
    (out_dir / "README.md").write_text(
        f"# {args.start_index:02d} 报纸页\n\n本目录属于藏语 `{version_name}`，由报纸页 JSON 结构树生成，共 6 张。\n\n- `html/`\n- `images/`\n- `labels/`\n- `metadata/`\n- `reports/`\n",
        encoding="utf-8",
    )
