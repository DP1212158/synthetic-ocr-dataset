...
# Keep standard Tibetan tsheg/shad punctuation legal: U+0F0B, U+0F0D, U+0F0E.
...
def main() -> int:
    ...
    for item in render_manifest:
        ...
        for block in label.get("blocks", []):
            ...
            if TIBETAN_SMALL_MARK_RE.search(text):
                small_mark_blocks += 1
            if c["x_min"] < 0 or c["y_min"] < 0 or c["x_max"] > width + 1 or c["y_max"] > height + 1:
                out_of_bounds += 1
            if (c["x_max"] - c["x_min"]) < 5 or (c["y_max"] - c["y_min"]) < 5:
                tiny_boxes += 1
    ...
    category_rows = "\n".join(
        f"| {category} | {stats['total']} | {stats['passed_basic']} | {stats['total_blocks']} | {stats['overflow_blocks']} | {stats['out_of_bounds_blocks']} | {stats['cyrillic_blocks']} | {stats['tibetan_small_mark_blocks']} |"
        for category, stats in category_summary.items()
    )
    md_rows = "\n".join(
        f"| {item['document_id']} | {item['blocks']} | {item['overflow_blocks']} | "
        f"{item['out_of_bounds_blocks']} | {item['cyrillic_blocks']} | {item['tibetan_small_mark_blocks']} | {item['min_blocks_required']} | {item['nonblank_score']} | {'通过' if item['pass_basic'] else '需检查'} |"
        for item in qa_items
    )
    md = f"""# 表格文档合成样本质检报告
...
| 类别 | 图片数 | 通过数 | 标签块 | DOM溢出 | 越界框 | 西里尔残留 | 小圆圈残留 |
|---|---:|---:|---:|---:|---:|---:|---:|
{category_rows}

## 明细

| 样本 | 标签块 | DOM溢出 | 越界框 | 西里尔残留 | 小圆圈残留 | 最小块数阈值 | 非空分数 | 结论 |
|---|---:|---:|---:|---:|---:|---:|---:|---|
{md_rows}
