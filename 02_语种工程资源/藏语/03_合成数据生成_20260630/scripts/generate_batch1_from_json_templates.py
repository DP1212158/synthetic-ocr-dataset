#!/usr/bin/env python3
"""Generate batch-1 Tibetan layouts from category-specific JSON templates."""
...
def load_language_profile(path: Path | None) -> dict[str, Any]:
    ...
    profile.setdefault("script_note", "藏文横排文本，batch1 JSON 结构树试点")
...
def select_span(text: str, rng: random.Random, min_len: int, max_len: int) -> str:
    ...
    if len(parts) > 8:
        start = rng.randint(0, max(0, len(parts) - 5))
        out = ""
...
def base_css(page: dict[str, Any], theme: dict[str, str], font_family: str, base_size: int = 23) -> str:
    ...
    * {{ box-sizing: border-box; }}
    ...
    .page {{ width: {int(page['width'])}px; min-height: {int(page['height'])}px; margin: 0 auto; padding: 50px; background: {theme['paper']}; position: relative; overflow: hidden; }}
...
def certificate_renderer(b: Builder, template: dict[str, Any], records: list[dict[str, str]], rng: random.Random, doc: dict[str, Any], theme: dict[str, str]) -> str:
    ...
    if v == 1:
        body = f"""<section class="award-cert">
          ...
          <footer>{b.block('metadata', '2026-07-01', 'meta')}{b.block('seal', 'ཐམ་ག', 'seal-circle')}{b.block('metadata', 'སྤྲོད་མཁན', 'meta')}</footer>
        </section>"""
        ...
        fields = "".join(kv(b, label, record_title(records, rng, 8, 18), "proof-kv") for label in ["མིང", "ཡིག་ཨང", "སྤྲོད་ཚེས", "ར་སྤྲོད"])
        body = f"""<section class="formal-proof">
          ...
          <footer>{b.block('metadata', '2026-07-01', 'meta')}{b.block('seal', 'ཐམ་ག', 'stamp')}</footer>
        </section>"""
        extra = f""".formal-proof{{min-height:1280px;border-left:18px solid {theme['accent']};background:white;padding:44px 56px}}.formal-proof header{{border-bottom:2px solid {theme['line']};padding-bottom:16px}}.proof-body{{font-size:28px;line-height:1.62;margin:36px 0}}.proof-grid{{display:grid;grid-template-columns:1fr 1fr;gap:14px}}.proof-kv{{border:1.4px solid {theme['line']};padding:12px;background:{theme['soft']}}}.formal-proof footer{{display:flex;justify-content:flex-end;gap:34px;align-items:center;margin-top:54px}}"""
    elif v == 3:
        body = f"""<section class="ornamental-cert">
          ...
          <div class="seal-row">{b.block('seal', 'ཐམ་ག', 'seal-circle')}{b.block('metadata', '2026-07-01', 'meta')}</div></div>
        </section>"""
        ...
        body = f"""<section class="credential-card">
          <aside>{b.block('metadata', f"ID {9000+v}", 'meta')}{b.block('seal', 'ཐམ་ག', 'seal-circle')}</aside>
        ...
        body = f"""<section class="proof-list">{b.block('metadata', f"No C-{3000+v:04d}", 'meta')}{b.block('document_title', doc['title'], 'doc-title')}{rows}<footer>{b.block('metadata', '2026-07-01', 'meta')}{b.block('seal', 'ཐམ་ག', 'stamp')}</footer></section>"""
    ...
    return wrap(doc, body, base_css(page, theme, doc["font_family"], 24) + extra, page, "certificate-page")
...
def exam_renderer(b: Builder, template: dict[str, Any], records: list[dict[str, str]], rng: random.Random, doc: dict[str, Any], theme: dict[str, str]) -> str:
    ...
    page = template["page"]
    if v == 1:
        body = f"""<header class="exam-head">{b.block('document_title', doc['title'], 'doc-title')}{b.block('metadata', 'རྒྱུགས་ཡུན 90 min    Score 100', 'meta')}</header><div class="student-line">{kv(b,'མིང','________')}{kv(b,'ཨང་རྟགས','________')}{kv(b,'ཁང་མིག','________')}</div><main class="exam-cols">{mcq(b, records, rng, 1, 14)}</main>"""
        ...
        body = f"""<header class="answer-head">{b.block('document_title', doc['title'], 'doc-title')}{b.block('metadata','ANSWER SHEET','meta')}</header><section class="student-line">{kv(b,'མིང','________')}{kv(b,'ཨང','________')}</section><main class="answer-sheet">{bubbles}</main><section class="short-answer">{b.block('section_title','ལན་ཐུང','section-title')}{''.join(b.block('answer_area',' ','answer-line') for _ in range(5))}</section>"""
        ...
        body = f"""<header class="essay-head">{b.block('document_title', doc['title'], 'doc-title')}{b.block('metadata','རྩོམ་ཡིག་དྲི་ཤོག','meta')}</header><main class="essay-layout"><section>{questions}</section><aside class="rubric-box">{b.block('section_title','དཔྱད་གཞི','section-title')}{rubric}</aside></main>{b.block('metadata','སྐར་གྲངས་དང་དཔྱད་མཆན་ཁག་རྒྱུགས་ཤོག་མཇུག་ཏུ་བཀོད།','meta essay-foot')}"""
        ...
        body = f"""<header class="score-head">{b.block('document_title', doc['title'], 'doc-title')}{b.block('metadata','ཐོབ་སྐར་ཞིབ་བཤེར','meta')}</header><main class="score-layout"><section>{mcq(b, records, rng, 1, 8)}</section><aside>{simple_table(b,['No','དྲི་བ','སྐར'],rows)}</aside></main>"""
        ...
        body = f"""<header class="oral-head">{b.block('metadata','ངག་རྒྱུགས / ལག་ལེན','meta')}{b.block('document_title', doc['title'], 'doc-title')}</header><main class="oral-grid">{cards}</main>"""
    ...
    return wrap(doc, body, base_css(page, theme, doc["font_family"], 22) + extra, page, "exam-page")
...
def sign_renderer(b: Builder, template: dict[str, Any], records: list[dict[str, str]], rng: random.Random, doc: dict[str, Any], theme: dict[str, str]) -> str:
    ...
    page = template["page"]
    if v == 1:
        arrows = "".join(b.block("metadata", label, "route-chip") for label in ["A-01", "B-02", "ལམ", "ཁ་ཕྱོགས"])
        body = f"""<section class="street-scene"><div class="sign-board">{b.block('document_title', record_title(records,rng,10,24), 'sign-title')}{b.block('metadata','ལམ་བྱང','meta')}{b.block('paragraph', record_text(records,rng,60,130), 'sign-copy')}<div class="route-row">{arrows}</div></div></section>"""
        ...
        body = f"""<section class="wall-board">{notes}<aside>{b.block('image','','image-box')}{b.block('metadata','གྱང་ངོས་ཡི་གེ','meta')}</aside></section>"""
    ...
    return wrap(doc, body, base_css(page, theme, doc["font_family"], 24) + extra, page, "sign-page")
...
def manifest_item(
    *,
    ...
    return {
        ...
        "template_type": cat["category_id"],
        "template_source": str(templates_json.resolve()),
        "generator": "batch1_json_renderer_bo_v13",
        "doc_no": doc_no,
        "date": now_local()[:10],
...
def main() -> int:
    ...
    spec = json.loads(templates_json.read_text(encoding="utf-8"))
    validate_spec(spec)

    rng = random.Random(args.seed)
    version_name = args.version_name or (Path(args.output_root).name if args.output_root else "v13")
    ...
    if args.output_root:
        ...
        for offset, cat in enumerate(spec["categories"]):
            ...
            (folder / "README.md").write_text(
                f"# {args.start_index + offset:02d} {category_cn}\n\n本目录属于藏语 `{version_name}`，由分类 JSON 结构树生成，共 6 张。\n\n- `html/`\n- `images/`\n- `labels/`\n- `metadata/`\n- `reports/`\n",
                encoding="utf-8",
            )
