...
def summarize(items: list[dict[str, Any]], args: argparse.Namespace) -> dict[str, Any]:
    ...
    for item in items:
        ...
        if item["lower_half_block_ratio"] < args.min_lower_half_block_ratio:
...
def write_report(summary: dict[str, Any], out_dir: Path) -> None:
    ...
    warning_rows = "\n".join(
        f"| {item['document_id']} | {item.get('category')} | {item['bottom_blank_ratio']} | {item['content_height_ratio']} | {item['lower_half_block_ratio']} | {', '.join(item['reasons'])} |"
