...
WARNING_ISSUES = {
    "long_tibetan_without_tsheg",
    "long_tibetan_without_shad",
...
def make_report(rows: list[dict[str, Any]], out_dir: Path, source_name: str, strict_labels: bool) -> dict[str, Any]:
    ...
    hard_issue_rows = [row for row in analyzed if any(issue in HARD_ISSUES for issue in row["issues"])]
    warning_rows = [
        row
        for row in analyzed
        if row["issues"] and not any(issue in HARD_ISSUES for issue in row["issues"])
    ]
    summary = {
        ...
        "warning_blocks": len(warning_rows),
        ...
        "samples": [
            ...
            for row in (hard_issue_rows + warning_rows)[:80]
        ],
    }
...
- 硬失败文本块数：{summary['hard_issue_blocks']}
- 抽检警告文本块数：{summary['warning_blocks']}
...
这份报告只能做工程级文本规范检查，不能替代母语专家对语义、语序和用词自然度的最终判断。`latin_residue`、`cjk_residue`、`box_or_replacement_chars`、PUA 和其他非藏文脚本残留按硬失败处理；`long_tibetan_without_shad`、`long_tibetan_without_tsheg` 和 `suspicious_syllables` 按人工抽检警告处理。
"""
