#!/usr/bin/env python3
"""Generate v12 reading-page categories from structured JSON templates."""
...
def load_language_profile(path: Path | None) -> dict[str, Any]:
    ...
    profile.setdefault("script_note", "藏文横排文本，v12 JSON 结构树分类生成")
...
def base_css(page: dict[str, Any], theme: dict[str, str], font_family: str, base: int = 21) -> str:
...
def book_renderer(b: Builder, tpl: dict[str, Any], records: list[dict[str, str]], rng: random.Random, doc: dict[str, Any], theme: dict[str, str]) -> str:
    ...
    page = tpl["page"]
    if v == 1:
        body = f"""<header class="running-header">{b.block('metadata', 'དེབ་ཀྱི་ནང་དོན', 'meta')}{b.block('metadata', '023', 'meta')}</header>
        ...
        body = f"""<header class="running-header">{b.block('metadata', 'རི་མོ་དང་འགྲེལ་བ', 'meta')}{b.block('metadata', '031', 'meta')}</header>
        ...
        body = f"""<header class="running-header">{b.block('metadata', 'ཀློག་དེབ', 'meta')}{b.block('metadata', '042', 'meta')}</header>
        ...
        body = f"""{b.block('document_title', 'དཀར་ཆག', 'contents-title')}<section class="toc">{''.join(entries)}</section>"""
        ...
        extracts = []
        for i in range(8):
            extracts.append(f'<section class="extract">{b.block("section_title", record_title(records, rng, 8, 22), "section-title")}{b.block("quote", record_text(records, rng, 100, 170), "quote")}{b.block("metadata", "འབྱུང་ཁུངས " + str(i + 1), "meta")}</section>')
        body = f"""{b.block('document_title', 'འདེམས་ཀློག', 'doc-title')}<div class="rule"></div><section class="extracts">{''.join(extracts)}</section>{b.block('page_number', '56', 'folio')}"""
    ...
    return wrap(doc, body, base_css(page, theme, doc["font_family"], 21) + extra, "book-page")
...
def textbook_renderer(b: Builder, tpl: dict[str, Any], records: list[dict[str, str]], rng: random.Random, doc: dict[str, Any], theme: dict[str, str]) -> str:
    ...
    if v == 1:
        ...
        body = f"""<header class="lesson-head">{b.block('metadata', 'Lesson 01', 'lesson-no')}{b.block('document_title', record_title(records, rng, 10, 26), 'doc-title')}</header><main class="lesson-grid"><section>{points}{b.block('section_title', 'དཔེ་མཚོན', 'section-title')}{para_stack(b, records, rng, 5, 70, 130)}<div class="mini-ex-wrap">{exercises}</div></section><aside class="key-box">{b.block('section_title', 'གནད་འགག', 'section-title')}{para_stack(b, records, rng, 10, 38, 82, 'small')}</aside></main>{b.block('page_number', '12', 'folio')}"""
        ...
        body = f"""<header class="exercise-head">{b.block('document_title', 'སྦྱོང་ཚན', 'doc-title')}{b.block('metadata', 'ལན་འབྲི་ས་སྟོང་པ་བཞག་ཡོད', 'meta')}</header><main class="exercise-cols">{''.join(qs)}</main>{b.block('note', record_text(records, rng, 50, 90), 'hint')}{b.block('page_number', '18', 'folio')}"""
        extra = f""".exercise-head{{display:flex;justify-content:space-between;align-items:end;border-bottom:3px double {theme['line']};padding-bottom:14px;}}.exercise-cols{{columns:2;column-gap:28px;margin-top:16px;}}.exercise{{break-inside:avoid;margin-bottom:6px;}}.question{{font-size:15px;line-height:1.28;}}.answer-line{{height:28px;border-bottom:1.5px solid {theme['line']};}}.hint{{position:absolute;left:62px;right:62px;bottom:54px;background:{theme['soft']};padding:10px;font-size:15px;}}"""
    elif v == 3:
        ...
        body = f"""{b.block('document_title', record_title(records, rng, 10, 24), 'doc-title')}<section class="diagram-page"><div>{b.block('image', '', 'image-box')}{b.block('caption', record_text(records, rng, 36, 76), 'caption')}{para_stack(b, records, rng, 5, 54, 105, 'small')}</div><aside>{calls}</aside></section><section class="summary-box">{b.block('section_title', 'བསྡུས་དོན', 'section-title')}{para_stack(b, records, rng, 6, 60, 110, 'small')}</section><section class="concept-grid">{concept_cards}</section>"""
        ...
        body = f"""<header class="vocab-head">{b.block('document_title', 'ཐ་སྙད་འགྲེལ་བ', 'doc-title')}{b.block('metadata', 'A - B', 'meta')}</header><main class="terms">{entries}</main>{b.block('page_number', '27', 'folio')}"""
        ...
        body = f"""<header class="activity-head">{b.block('metadata', 'Activity', 'meta')}{b.block('document_title', record_title(records, rng, 10, 24), 'doc-title')}</header><main class="activity-grid"><section>{b.block('section_title', 'ལས་འགན', 'section-title')}{para_stack(b, records, rng, 5, 65, 115)}{b.block('section_title', 'གོ་རིམ', 'section-title')}{steps}</section><aside>{b.block('section_title', 'དགོས་མཁོ', 'section-title')}{para_stack(b, records, rng, 8, 30, 64, 'small')}{b.block('answer_area', ' ', 'observation')}</aside></main>"""
        ...
        body = f"""{b.block('document_title', 'སློབ་ཚན་བསྐྱར་སྦྱོང', 'doc-title')}<main class="review-layout"><section class="review-grid">{blocks}</section><aside class="check-box">{b.block('section_title', 'རང་ཞིབ', 'section-title')}{checks}</aside></main>{b.block('page_number', '38', 'folio')}"""
    ...
    return wrap(doc, body, base_css(page, theme, doc["font_family"], 21) + extra, "textbook-page")
...
def magazine_renderer(b: Builder, tpl: dict[str, Any], records: list[dict[str, str]], rng: random.Random, doc: dict[str, Any], theme: dict[str, str]) -> str:
    ...
    if v == 1:
        ...
        body = f"""{b.block('document_title', 'དཀར་ཆག', 'contents-title')}{b.block('metadata', 'Vol. 07', 'meta')}<main class="journal-contents">{entries}</main>"""
    ...
    return wrap(doc, body, base_css(page, theme, doc["font_family"], 21) + extra, "magazine-page")
...
def main() -> int:
    ...
    for offset, cat in enumerate(spec["categories"]):
            ...
            html_manifest.append(
                {
                    ...
                    "generator": "reading_pages_json_renderer_bo_v13",
        ...
        (folder / "README.md").write_text(
            f"# {args.start_index + offset:02d} {category_cn}\n\n本目录属于藏语 `{version_name}`，由阅读型页面 JSON 结构树生成，共 6 张。\n\n- `html/`\n- `images/`\n- `labels/`\n- `metadata/`\n- `reports/`\n",
            encoding="utf-8",
        )
