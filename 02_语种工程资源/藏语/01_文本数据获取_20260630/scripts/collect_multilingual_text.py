#!/usr/bin/env python3
"""Collect plain-text samples from a configurable public text source.
...
def script_profile(text: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "tibetan_ratio": round(char_ratio(text, TIBETAN_RE), 4),
        "latin_ratio": round(char_ratio(text, LATIN_RE), 4),
    }


def compile_script_regex(pattern: str) -> re.Pattern[str]:
...
def list_categorymembers_batch(
    endpoint: str,
    category_title: str,
    limit: int,
    timeout: int,
...
def build_summary(
    ...
    tibetan_ratios = [float(r.get("tibetan_ratio", 0)) for r in records]
    ...
    return {
        "run": {
            ...
            "script_regex": args.script_regex,
        ...
        "collection": {
            ...
            "avg_tibetan_ratio": round(statistics.mean(tibetan_ratios), 4) if tibetan_ratios else 0,
            "avg_latin_ratio": round(statistics.mean(latin_ratios), 4) if latin_ratios else 0,
        },
    }


def write_report(path: Path, summary: Dict[str, Any], records: List[Dict[str, Any]], min_script_ratio: float) -> None:
    ...
    elif run["language_code"] == "bo" and coll["avg_tibetan_ratio"] < min_script_ratio:
        script_warning = (
            "\n- 注意：本批样本的藏文字符占比偏低，说明来源中混入了较多非藏文内容。"
            "后续需要提高过滤阈值或改进站点选择。\n"
        )
    sample_lines = []
    for r in records[:5]:
...
- 平均藏文字符占比：{coll['avg_tibetan_ratio']}
...
def collect(args: argparse.Namespace) -> int:
    script_re = compile_script_regex(args.script_regex)
    ...
    while pages_seen < args.max_pages:
        ...
        if args.mode == "random":
            ...
            apcontinue = None
            cmcontinue = None
        elif args.mode == "categorymembers":
            try:
                pages, cmcontinue = list_categorymembers_batch(
                    args.endpoint,
                    args.category_title,
                    batch_limit,
                    args.timeout,
                ...
                errors.append({"stage": "categorymembers", "error": repr(exc), "time": now_utc(), "cmcontinue": cmcontinue})
                break
            apcontinue = None
        else:
            try:
        ...
        for page in fetched_pages:
            ...
            for idx, paragraph in enumerate(paragraphs):
                ...
                script_ratio = char_ratio(paragraph, script_re)
                if script_ratio < args.min_script_ratio:
                    continue
        ...
        if args.mode == "allpages" and not apcontinue:
            break
        if args.mode == "categorymembers" and not cmcontinue:
            break
    ...
    write_report(out_dir / "README.md", summary, records, args.min_script_ratio)
    return 0 if records else 2


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect public text samples through MediaWiki API.")
    parser.add_argument("--mode", choices=["allpages", "random", "categorymembers"], default="allpages")
    ...
    parser.add_argument("--source-name", default="wikipedia_api")
    parser.add_argument("--category-title", default="")
    parser.add_argument("--script-regex", default=r"[\u0400-\u04ff]")
    parser.add_argument("--min-script-ratio", type=float, default=0.35)
